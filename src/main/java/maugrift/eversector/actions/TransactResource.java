...
public class TransactResource implements Action
{
    ...
    public String canExecute(Ship actor)
    {
        ...
        if (resourceObj == null)
        {
            ...
            {
                return "The ship cannot store over " + Ship.MAX_EXPANDERS + " " + expander.getName().toLowerCase() +
                       "s.";
            }
        ...
        if (!resourceObj.canHold(quantity))
        {
            if (actor.isPlayer())
            {
                if (quantity > 0)
                {
                    return "Inadequate storage; have " + resourceObj.getCapacity() + ", need " +
                           (resourceObj.getAmount() + quantity) + ".";
                }

                return "Inadequate resources to sell; have " + resourceObj.getAmount() + ", need " + Math.abs(
                        quantity) + ".";
            }
        }
